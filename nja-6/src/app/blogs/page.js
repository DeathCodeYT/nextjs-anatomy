...
export default function Page() {
  ...
  useEffect(() => {
    fetch('https://jsonplaceholder.typicode.com/posts').then(res => res.json()).then(setPost)
  }, [])
  return <>
    <div className='flex flex-wrap gap-4'>

      {posts.map(p => (
        <Link href={`/blogs/${p.id}`} key={p.id} class="max-w-md mx-auto my-6 bg-[#1e1e1e] border border-[#333333] rounded-xl p-5 shadow-lg transition-all duration-300 hover:bg-[#252525] hover:border-[#444444] group">
